return {
  ...
  config = function()
    ...
    local function handle_local_server_data(data_stream)
      local success, json = pcall(vim.json.decode, data_stream)
      if success then
        if json.content then
          dingllm.write_string_at_cursor(json.content)
        end
      else
        print("non json " .. data_stream)
      end
    ...
    local function make_local_server_args(opts, prompt)
      local data = {
        prompt = prompt,
        n_predict = 128,
      }
    ...
    local function local_server()
      dingllm.invoke_llm_and_stream_into_editor({
        replace = false,
        system_prompt = system_prompt,
      }, make_local_server_args, handle_local_server_data)
    end

    local function local_server_replace()
      dingllm.invoke_llm_and_stream_into_editor({
        replace = true,
        system_prompt = system_prompt,
      }, make_local_server_args, handle_local_server_data)
    ...
    vim.keymap.set({ "n", "v" }, "<leader>m", local_server, { desc = "local server" })
    vim.keymap.set({ "n", "v" }, "<leader>M", local_server_replace, { desc = "local server replace" })
  end,
}
